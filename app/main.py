...
def safe_ping(host: str) -> dict:
        ...
        if sanitized_host and all(c.isalnum() or c in ('.', '-') for c in sanitized_host):
            try:
                output = subprocess.check_output(['ping', re.escape(sanitized_host)], stderr=subprocess.STDOUT, timeout=5)
...
@app.get('/ping')
def ping(host: str):
    # Validate the input using a regex pattern to allow only alphanumeric characters, dots, hyphens, and underscores
    import re
