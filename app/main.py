...
def safe_ping(host: str):
    ...
    try:
        result = subprocess.run(['ping', '-c 1', re.escape(host)], capture_output=True, text=True, timeout=5, check=True)
...
def safe_ping_shell(host: str):
    ...
    if host not in allowed_hosts:
        return "Invalid input"
    try:
        result = subprocess.run(f'ping -c 1 {re.escape(host)}', capture_output=True, text=True, timeout=5, check=True, shell=False)
        return result.stdout
    except subprocess.CalledProcessError as e:
        return str(e)
