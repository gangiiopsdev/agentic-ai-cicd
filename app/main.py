...
def safe_ping_shell(host: str):
    # Safe implementation using subprocess.run with shell=True for demonstration purposes only
    result = subprocess.run(f'ping {host}', capture_output=True, text=True, shell=True)
    return result
