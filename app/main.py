...
def safe_run(command, *args, **kwargs):
        ...
        try:
            result = subprocess.run(command, check=True, input=b'', stdout=subprocess.PIPE, stderr=subprocess.PIPE, *args, **kwargs)
...
def ping(host):
        ...
        command = ['ping', '-c', '1', shlex.quote(host)]  # Use shlex.quote to sanitize input
