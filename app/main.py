...
class PingEndpoint:
    ...
    async def ping(self, host):
        try:
            # Use a whitelist approach to allow only valid IP addresses and hostnames
            pattern = re.compile(r'^[a-zA-Z0-9.-]{1,253}$')
            if not pattern.match(host):
                raise ValueError('Invalid input')
            # Sanitize the input using subprocess.quote
            sanitized_host = subprocess.quote(host)
            result = await self.ping_command.execute(sanitized_host)
