...
def ping_route(credentials: HTTPBasicCredentials = Depends(security)):
    ...
    try:
        output = subprocess.run(['/usr/bin/ping', '-c', '1', subprocess.check_output(f'echo {host}', shell=True).decode().strip()], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
